...
function cappedBar(x: number, y: number, height: number, rounded: boolean): string {
	...
	return [`M${x} ${bottom}`, `V${y + r}`, `Q${x} ${y} ${x + r} ${y}`, `H${x + BAR - r}`, `Q${x + BAR} ${y} ${x + BAR} ${y + r}`, `V${bottom}`, "Z"].join("");
}
	...
	return (
		<figure className="rounded-lg border border-zinc-300 bg-white p-4">
			...
			<div className="relative">
				<svg
					viewBox={`0 0 ${width} ${PLOT_HEIGHT + 30}`}
				...
				>
					{ticks.map((tick) => {
						const y = PLOT_HEIGHT - scale(tick) + 10;
						return (
							<g key={tick}>
								<line x1={AXIS_WIDTH} x2={width} y1={y} y2={y} stroke={tick === 0 ? CHART_INK.baseline : CHART_INK.gridline} strokeWidth={1} />
								<text x={AXIS_WIDTH - 5} y={y} textAnchor="end" dominantBaseline="middle" fontSize={8} fill={CHART_INK.muted}>
									{tick}
								</text>
							</g>
						);
					...
					{daily.map((day, index) => {
						...
						return (
							<g key={day.date} onMouseEnter={() => setHover(index)} onMouseLeave={() => setHover(null)}>
								...
								<rect x={x} y={0} width={COLUMN} height={PLOT_HEIGHT} fill={hover === index ? "rgba(11,11,11,0.05)" : "transparent"} />
								{present.map((taskType, position) => {
									const height = Math.max(1.5, scale(day.byType[taskType]));
									const top = cursor - height;
									cursor = top - GAP;
									return <path key={taskType} d={cappedBar(x + GAP / 2, top, height, position === present.length - 1)} fill={SERIES_COLOUR[taskType]} />;
								})}
							</g>
						);
					})}

					<text x={AXIS_WIDTH} y={PLOT_HEIGHT + 22} fontSize={8} fill={CHART_INK.muted}>
						{daily[0]?.date.slice(5)}
					</text>
					<text x={width} y={PLOT_HEIGHT + 22} textAnchor="end" fontSize={8} fill={CHART_INK.muted}>
				...
				{active && active.total > 0 && (
					...
					>
						...
						{SERIES_ORDER.filter((t) => (active.byType[t] ?? 0) > 0).map((taskType) => (
							<p key={taskType} className="flex items-center gap-1.5">
								<span aria-hidden className="h-2 w-2 rounded-sm" style={{ background: SERIES_COLOUR[taskType] }} />
			...
			<ul className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-zinc-600">
				{SERIES_ORDER.map((taskType) => (
					<li key={taskType} className="flex items-center gap-1.5">
						<span aria-hidden className="h-2.5 w-2.5 rounded-sm" style={{ background: SERIES_COLOUR[taskType] }} />
